  ...
  <div>
    ...
    <el-table :data="tableData" stripe style="width: 100%">
      <el-table-column v-for="(col, index) in tableColumns" :key="index" :prop="col.prop" :label="col.label"
        :width="col.width" align="center"></el-table-column>
...
import { getRawMaterial, getConnection, getWeld, getBeton, deleteRawById, deleteConnectionById,deleteWeldById,deleteBetonById } from "@/api/ledger";
export default {
  data() {
    return {
      searchProjectName: "",
  ...
  methods: {
    ...
    async getWeldAllData() {
      console.log("发请求，获取钢筋焊接的数据");
      const user = getUser();
      try {
        const res = await getWeld(
          this.curPage, // 对应 pageNum（当前页码）
          this.limit,   // 对应 pageSize（每页条数）
          user.userId,  // 用户Id
          "",           // projectName（搜索条件，暂时为空）
          "",           // subProjectName（搜索条件，暂时为空）
          ""            // usePart（搜索条件，暂时为空）         
        );
        if (res.code == 200) {
          console.log("获取数据状态", res);
    ...
    deleteItem(row) {
      if (this.category === "钢筋原材") {
        this.deleteRawById(row);
      } else if (this.category === "钢筋机械连接") {
        this.deleteConnectionById(row);
      } else if (this.category === "钢筋焊接") {
        this.deleteWeldById(row);
      } else if (this.category === "混凝土强度") {
        this.deleteBetonById(row);
      } else {
        this.message.error(`没有 ${category} 类型的检验批，出错了！`);
      }
    },
    async deleteRawById(row){
      try {
        const res = await deleteRawById(row.dataId)
        if (res.code == 200) {
    ...
    async deleteConnectionById(row){
      try {
        const res = await deleteConnectionById(row.dataId)
        if (res.code == 200) {
          this.tableData = this.tableData.filter(item => item.dataId !== row.dataId);
          this.total = Math.max(0, this.total - 1);
          this.$message({
    ...
    async deleteWeldById(row){
      try {
        const res = await deleteWeldById(row.dataId)
        if (res.code == 200) {
          this.tableData = this.tableData.filter(item => item.dataId !== row.dataId);
          this.total = Math.max(0, this.total - 1);
          this.$message({
    ...
    async deleteBetonById(row){
      try {
        const res = await deleteBetonById(row.dataId)
        if (res.code == 200) {
          this.tableData = this.tableData.filter(item => item.dataId !== row.dataId);
          this.total = Math.max(0, this.total - 1);
          this.$message({
